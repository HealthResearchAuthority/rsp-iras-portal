...
public class WorkspacePermissionsPolicyProvider(IOptions<AuthorizationOptions> options) : IAuthorizationPolicyProvider
{
    ...
    // The commented code below shows examples of other policies that might be registered
    // in a typical authorization configuration. These are left as documentation/reference.
    //
    // var policy = new AuthorizationPolicyBuilder()
    ...
    // builder
    //     .AddPolicy("IsReviewer", policy => policy.RequireRole(RoleNames.reviewer))
    //         .AddPolicy("IsSystemAdministrator", policy => policy.RequireRole(RoleNames.systemAdministrator))
    //         .AddPolicy("IsApplicant", policy => policy.RequireRole(RoleNames.applicant))
    //         .AddPolicy("IsSponsor", policy => policy.RequireRole(RoleNames.sponsor))
    //         .AddPolicy("IsStudyWideReviewer", policy => policy.RequireRole(RoleNames.studyWideReviewer))
    //         .AddPolicy("IsTeamManager", policy => policy.RequireRole(RoleNames.teamManager))
    //         .AddPolicy("IsWorkflowCoordinator", policy => policy.RequireRole(RoleNames.workflowCoordinator))
    //         .AddPolicy("IsBackstageUser", p => p.RequireRole(
    //             RoleNames.applicant,
    //             RoleNames.systemAdministrator,
    //             RoleNames.studyWideReviewer,
    //             RoleNames.teamManager,
    //             RoleNames.workflowCoordinator));
}