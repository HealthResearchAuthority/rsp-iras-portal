    ...
    )
    {
        foreach (var change in modificationChanges!.OrderByDescending(c => c.CreatedDate))
        {
            ...
            var changeModel = new ModificationChangeModel
            {
                ...
                ModificationType = (
                    ranking?.ModificationType?.Substantiality ?? string.Empty,
                    ranking?.ModificationType?.Order ?? 0
                ),
                Categorisation = (
                    ranking?.Categorisation?.Category ?? string.Empty,
                    ranking?.Categorisation?.Order ?? 0
                ),
                ReviewType = ranking?.ReviewType ?? string.Empty,
                ...
                ShowApplicabilityQuestions = specificAreaOfChange?.ShowApplicabilityQuestions ?? false,
