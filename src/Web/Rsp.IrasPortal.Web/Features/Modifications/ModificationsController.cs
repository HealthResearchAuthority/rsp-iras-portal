...

{
    ...
    public async Task<IActionResult> ConfirmModificationJourney(AreaOfChangeViewModel model, bool saveForLater = false)
    {
        ...
        {
            (bool flowControl, IActionResult value) = await CheckIfSelectedChangeIsAllowed(model, selectedChange);
            if (!flowControl)
            {
                return value;
            }
    ...
    AnswerModel selectedChange)
    {
        ...
        if (!isValid)
        {
            return (false, View("SelectedModificationChangeErrorPage", (model, selectedChange)));
        }
        ...
        AnswerModel selectedChange)
    {
        ...
        var hasSpecificChangesWithNoOtherChangesRules = previousSelections.Any(id => specificChangesWithNoOtherChangesRules.Contains(id));
        var hasOtherChanges = previousSelections.Any();

        //selecting 'no-other-changes' change after any other changes or already have 'no-other-changes' change added
        return !((isCurrentSpecificChangeWithNoOtherChangesRules && hasOtherChanges) || hasSpecificChangesWithNoOtherChangesRules);
    }
