...

{
     ...
     AreaOfChangeViewModel model, AnswerModel selectedChange)
    {
        ...
        var specificAreaOfChangeOptionNameKey = TempData.Peek(TempDataKeys.SpecificAreaOfChangeOptionNameKey) is string json
            ...
            : new List<string>();
        bool isHaltNow = selectedChange.AutoGeneratedId == AreasOfChange.ProjectHalt;
        bool isRestartNow = selectedChange.AutoGeneratedId == AreasOfChange.ProjectRestart;
        bool haltAlreadySelected = specificAreaOfChangeOptionNameKey.Contains(AreasOfChange.ProjectHalt);
        bool restartAlreadySelected = specificAreaOfChangeOptionNameKey.Contains(AreasOfChange.ProjectRestart);
        bool hasOtherChanges = specificAreaOfChangeOptionNameKey.Any(x => x != AreasOfChange.ProjectHalt);
        ...
        // Alreadt exist HALT/Restart can't add another one
        ...
        if (!specificAreaOfChangeOptionNameKey.Contains(selectedChange.AutoGeneratedId))
            specificAreaOfChangeOptionNameKey.Add(selectedChange.AutoGeneratedId);
