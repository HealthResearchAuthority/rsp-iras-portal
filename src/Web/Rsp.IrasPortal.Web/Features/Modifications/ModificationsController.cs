...

{
    ...
    public async Task<IActionResult> AreaOfChange(Guid? projectModificationId, string projectRecordId)
    {
        ...
        if (!await featureManager.IsEnabledAsync(FeatureFlags.ChangeOfSponsorOrganisation))
        {
            startingQuestions?.AreasOfChange.RemoveAll(a => a.AutoGeneratedId == AreasOfChange.ChangeOfPrimarySponsor);
        }
    ...
    public async Task<IActionResult> ConfirmModificationJourney(AreaOfChangeViewModel model, bool saveForLater = false)
    {
        ...
        //Restrict if project restart if project is in active status
        ...
        {
            (bool flowControl, IActionResult value) = await CheckIsHaltProject(model, selectedChange);
            if (!flowControl)
            {
                return value;
            }
    ...
    private async Task<(bool flowControl, IActionResult value)> CheckIsHaltProject(
    ...
    AnswerModel selectedChange)
    {
        ...
        var isValid = EvaluateSpecificAreaOfChange(previousSelections, selectedChange);
        if (!isValid)
        {
    ...
    /// <summary>
    /// Encapsulates all the HALT/RESTART validation rules
    /// </summary>
    private bool EvaluateSpecificAreaOfChange(
        IReadOnlyCollection<string> previousSelections,
        AnswerModel selectedChange)
    {
        var isHaltNow = selectedChange?.AutoGeneratedId == AreasOfChange.ProjectHalt;
        var isRestartNow = selectedChange?.AutoGeneratedId == AreasOfChange.ProjectRestart;
        var haltAlreadySelected = previousSelections.Contains(AreasOfChange.ProjectHalt);
        var restartAlreadySelected = previousSelections.Contains(AreasOfChange.ProjectRestart);
        var hasOtherChanges = previousSelections.Any();

        //selecting HALT after other changes (when not already selected)
        ...
        if (!isHaltNow && haltAlreadySelected && !isRestartNow)
        {
            return false;
        }

        // prevent duplicates for HALT/RESTART
        if ((isHaltNow && haltAlreadySelected) || (isRestartNow && restartAlreadySelected))
        {
            return false;
        }
    ...
    private async Task<List<string>> BuildSelectionsFromDatabaseAsync(AreaOfChangeViewModel model)
    {
        ...
        // Assuming areaOfChange is the AutoGeneratedId / string key you compare against
        foreach (var item in response.Content)
        {
            if (!string.IsNullOrWhiteSpace(item?.AreaOfChange))
            {
                result.Add(item.AreaOfChange);
            }
        }
