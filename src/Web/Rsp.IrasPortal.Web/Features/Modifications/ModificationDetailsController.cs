...
) : ModificationsControllerBase(respondentService, projectModificationsService, cmsQuestionsetService)
{
    ...
    public async Task<IActionResult> ModificationDetails(string projectRecordId, string irasId, string shortTitle, Guid projectModificationId)
    {
        ...
        TempData[TempDataKeys.ProjectModification.ProjectModificationIdentifier] = viewModel.ModificationIdentifier;
        TempData[TempDataKeys.ProjectModification.ProjectModificationId] = viewModel.ModificationId;

        var (changesResult, initialQuestions, modificationChanges) = await GetModificationChanges(viewModel);
        ...
        viewModel.ModificationChanges = await UpdateModificationChanges(projectRecordId, viewModel.ModificationChanges);

        // Set the 'ready for submission' flag if all changes are ready
        if (viewModel.ModificationChanges.All(c => c.ChangeStatus == ModificationStatus.ChangeReadyForSubmission))
        {
