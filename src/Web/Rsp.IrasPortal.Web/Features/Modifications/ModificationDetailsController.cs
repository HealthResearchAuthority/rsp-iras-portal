...
) : Controller
{
    ...
    private async Task<IEnumerable<ModificationChangeModel>> UpdateModificationChanges(string projectRecordId, List<ModificationChangeModel> modificationChanges)
    {
        foreach (var modificationChange in modificationChanges)
        {
            ...
            // see if showAnswerOn is configured to show on ModificationDetails
            if (surfacingQuestion?.ShowAnswerOn.Contains(nameof(ModificationDetails), StringComparison.OrdinalIgnoreCase) == true)
            {
                static string? ParseDate(string? plannedDate)
                {
                    if (!string.IsNullOrEmpty(plannedDate) &&
                        DateTime.TryParseExact(plannedDate, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date))
                    {
                ...
                modificationChange.SpecificChangeAnswer = surfacingQuestion.DataType.ToLower() switch
                {
                    "date" => ParseDate(surfacingQuestion.AnswerText),
                    "text" or "email" => surfacingQuestion.AnswerText,
                    "checkbox" => string.Join(", ", surfacingQuestion.Answers.Select(answer => answer.AnswerText)),
                    "radio button" or "boolean" or "look-up list" or "dropdown" => surfacingQuestion.SelectedOption,
                    _ => ""
                };
            }
        }
