...
) : IProjectClosuresService
{
    ...
    {
        var apiResponse = await projectClosuresServiceClient.GetProjectClosuresBySponsorOrganisationUserId(sponsorOrganisationUserId, searchQuery, rtsId, pageNumber, pageSize, sortField, sortDirection);
        return apiResponse.ToServiceResponse();
    }
    ...
    {
        var apiResponse = await projectClosuresServiceClient.GetProjectClosuresBySponsorOrganisationUserIdWithoutPaging(sponsorOrganisationUserId, rtsId, searchQuery);
        return apiResponse.ToServiceResponse();
    }
