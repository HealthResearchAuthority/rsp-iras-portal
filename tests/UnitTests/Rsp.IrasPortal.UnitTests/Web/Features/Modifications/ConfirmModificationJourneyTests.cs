...
public class ConfirmModificationJourneyTests : TestServiceBase<ModificationsController>
{
    ...
    [Fact]
    public async Task ConfirmModificationJourney_Should_Redirect_To_ProjectHaltWarning_When_Its_Halt()
    {
        // Arrange
        var area = new List<AreaOfChangeDto>()
        {
            new AreaOfChangeDto{
            AutoGeneratedId = "A1",
            ...
            SpecificAreasOfChange =
                   [
                       new() { AutoGeneratedId = "192152cb-db19-447c-b410-aa394f1de54c", OptionName = "Temporary halt to a project" },
                   ]
            },
        };

        var modificationId = Guid.NewGuid();
        var model = new AreaOfChangeViewModel { ProjectRecordId = "PR1", AreaOfChangeId = "A1", SpecificChangeId = "192152cb-db19-447c-b410-aa394f1de54c" };
        SetupTempData(model, area, modificationId);
    ...
    public async Task ConfirmModificationJourney_Should_Redirect_To_AreaOfChange_When_Other_AreaOfChange_Exist()
    {
        ...
        var modificationId = Guid.NewGuid();
        var model = new AreaOfChangeViewModel { ProjectRecordId = "PR1", AreaOfChangeId = "A1", SpecificChangeId = "S1" };
        var existing = new List<string> { "192152cb-db19-447c-b410-aa394f1de54c" };
    ...
    [Fact]
    public async Task ConfirmModificationJourney_Should_Redirect_To_AreaOfChange_When_ProjectRestart_Warning()
    {
        // Arrange
        var modificationId = Guid.NewGuid();
        ...
        var temp = new TempDataDictionary(ctx, Mock.Of<ITempDataProvider>())
        {
            ...
            [TempDataKeys.ProjectRecordId] = model.ProjectRecordId ?? "PR1",
            [TempDataKeys.SpecificAreaOfChangeOptionNameKey] = JsonSerializer.Serialize(existing),
            [TempDataKeys.ProjectRecordStatus] = "Active"
        };
        temp[TempDataKeys.ProjectModification.ProjectModificationChangeId] = modificationId;
        Sut.TempData = temp;
        Sut.ControllerContext = new ControllerContext { HttpContext = ctx };
       ...
       .ReturnsAsync(new ValidationResult());
        ...
        redirectResult.ViewName.ShouldBe("ProjectRestartWarning");
    }

    [Fact]
    public async Task ConfirmModificationJourney_Should_Redirect_ProjectRestartWarning()
    {
        // Arrange
        var modificationId = Guid.NewGuid();
        var model = new AreaOfChangeViewModel { ProjectRecordId = "PR1", AreaOfChangeId = "A1", SpecificChangeId = "0c715e3f-22bd-47fb-9a37-710e0248b761" };
    ...
    [Fact]
    public async Task ConfirmModificationJourney_Should_Redirect_ProjectHaltWarning()
    {
        // Arrange
        var modificationId = Guid.NewGuid();
        var model = new AreaOfChangeViewModel { ProjectRecordId = "PR1", AreaOfChangeId = "A1", SpecificChangeId = "192152cb-db19-447c-b410-aa394f1de54c" };
            ...
            new AreaOfChangeDto
        {
            ...
            SpecificAreasOfChange =
            [
                new() { AutoGeneratedId = "192152cb-db19-447c-b410-aa394f1de54c", OptionName = "Spec 1" }
            ]
        }
        };
        var temp = new TempDataDictionary(ctx, Mock.Of<ITempDataProvider>())
        {
            ...
            [TempDataKeys.ShortProjectTitle] = "Test project",
            [TempDataKeys.IrasId] = 123456,
            [TempDataKeys.ProjectModification.AreaOfChanges] = JsonSerializer.Serialize(area),
            [TempDataKeys.ProjectRecordId] = model.ProjectRecordId ?? "PR1",
            [TempDataKeys.SpecificAreaOfChangeOptionNameKey] = JsonSerializer.Serialize(existing),
            [TempDataKeys.ProjectRecordStatus] = "Project halt"
        };
    ...
    private static List<AreaOfChangeDto> AreasBuild() =>
    [
        new AreaOfChangeDto
        {
            AutoGeneratedId = "A1",
            ...
            SpecificAreasOfChange =
            [
                new() { AutoGeneratedId = "0c715e3f-22bd-47fb-9a37-710e0248b761", OptionName = "Spec 1" },
                new() { AutoGeneratedId = "192152cb-db19-447c-b410-aa394f1de54c", OptionName = "Spec 2" }
            ]
        }
    ];
