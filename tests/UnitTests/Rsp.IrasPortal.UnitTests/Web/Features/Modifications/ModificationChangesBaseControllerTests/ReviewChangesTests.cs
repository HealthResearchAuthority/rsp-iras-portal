...
public class ReviewChangesTests : TestServiceBase<ModificationChangesBaseController>
{
    ...
    [Theory, AutoData]
    public async Task ReviewChanges_Returns_Error_When_InitialQuestions_Fail(string projectRecordId, Guid specificAreaOfChangeId, Guid modificationChangeId)
    {
        // Arrange
        _respondentService
            .Setup(s => s.GetModificationChangeAnswers(It.IsAny<Guid>(), It.IsAny<string>()))
        ...
        _cmsService
            ...
            {
                StatusCode = HttpStatusCode.OK,
                Content = BuildQuestionSet("SEC1", "CAT1", "ReviewChanges",
                    new QuestionModel { Id = "QCMS1", QuestionId = "Q1", AnswerDataType = "Text", QuestionFormat = "text", CategoryId = "CAT1", Sequence = 1, SectionSequence = 1 })
            });

        _cmsService
            .Setup(s => s.GetInitialModificationQuestions())
            .ReturnsAsync(new ServiceResponse<StartingQuestionsDto> { StatusCode = HttpStatusCode.BadRequest, Error = "fail" });
    ...
    [Fact]
    public async Task ReviewChanges_WithReviseChange_AndAddNewSites_ValidatesParticipatingOrganisationQuestionnaire_AndExcludesOrganisationSection()
    {
        // Arrange
        var projectRecordId = "PR1";
        var specificAreaOfChangeId = Guid.Parse(SpecificAreasOfChange.AddNewSites);
        var modificationChangeId = Guid.NewGuid();
        var participatingOrganisationId = Guid.NewGuid();

        _respondentService
            .Setup(s => s.GetModificationChangeAnswers(modificationChangeId, projectRecordId))
            .ReturnsAsync(new ServiceResponse<IEnumerable<RespondentAnswerDto>>
            {
                StatusCode = HttpStatusCode.OK,
                Content = [new RespondentAnswerDto { QuestionId = "Q-MAIN", AnswerText = "main-answer" }]
            });

        var journey = new CmsQuestionSetResponse
        {
            Sections =
            [
                new SectionModel
                {
                    ...
                    CategoryId = "CAT1",
                    StaticViewName = "ReviewChanges",
                    Questions = [new QuestionModel { Id = "Q-MAIN", QuestionId = "Q-MAIN", AnswerDataType = "Text", QuestionFormat = "text", CategoryId = "CAT1", Sequence = 1, SectionSequence = 1 }]
                },
                new SectionModel
                {
                    Id = "pom-participating-organisation-details",
                    SectionId = "pom-participating-organisation-details",
                    CategoryId = "CAT1",
                    Questions = [new QuestionModel { Id = "Q-ORG", QuestionId = "Q-ORG", AnswerDataType = "Text", QuestionFormat = "text", CategoryId = "CAT1", Sequence = 2, SectionSequence = 2 }]
                }
            ]
        };

        _cmsService
            .Setup(s => s.GetModificationsJourney(specificAreaOfChangeId.ToString()))
            .ReturnsAsync(new ServiceResponse<CmsQuestionSetResponse> { StatusCode = HttpStatusCode.OK, Content = journey });
                ...
                Content = new StartingQuestionsDto
                {
                    ...
                    [
                        new AreaOfChangeDto
                        {
                            SpecificAreasOfChange =
                            [
                                new AnswerModel { AutoGeneratedId = specificAreaOfChangeId.ToString(), OptionName = "Add new sites" }
                            ]
                        }
                    ]
                }
        ...
        _cmsService
            .Setup(s => s.GetModificationQuestionSet("SEC-REVIEW", null))
            .ReturnsAsync(new ServiceResponse<CmsQuestionSetResponse> { StatusCode = HttpStatusCode.OK, Content = journey });

        _cmsService
            .Setup(s => s.GetModificationPreviousQuestionSection("SEC-REVIEW", It.IsAny<string?>(), It.IsAny<string?>()))
            .ReturnsAsync(new ServiceResponse<QuestionSectionsResponse>
            {
                StatusCode = HttpStatusCode.OK,
                Content = new QuestionSectionsResponse { SectionId = "PREV", QuestionCategoryId = "CAT1" }
        ...
        _cmsService
            .Setup(s => s.GetModificationNextQuestionSection("SEC-REVIEW", It.IsAny<string?>(), It.IsAny<string?>()))
            .ReturnsAsync(new ServiceResponse<QuestionSectionsResponse>
            {
                StatusCode = HttpStatusCode.OK,
                Content = new QuestionSectionsResponse { SectionId = "NEXT", QuestionCategoryId = "CAT1" }
        ...
        _respondentService
            ...
            {
                StatusCode = HttpStatusCode.OK,
                Content = [new ParticipatingOrganisationDto { Id = participatingOrganisationId, OrganisationId = "ORG-1" }]
            });
                ...
                Content = new CmsQuestionSetResponse
                {
                    ...
                    [
                        new SectionModel
                        {
                            ...
                            SectionId = "pom-participating-organisation-details",
                            CategoryId = "CAT1",
                            Questions = [new QuestionModel { Id = "Q-ORG", QuestionId = "Q-ORG", AnswerDataType = "Text", QuestionFormat = "text", CategoryId = "CAT1", Sequence = 1, SectionSequence = 1 }]
                        }
                    ]
                }
            });
        ...
        _respondentService
            ...
            {
                StatusCode = HttpStatusCode.OK,
                Content = [new ParticipatingOrganisationAnswerDto { Id = Guid.NewGuid(), QuestionId = "Q-ORG", AnswerText = "org-answer" }]
            });
        ...
        // Act
        var result = await Sut.ReviewChanges(projectRecordId, specificAreaOfChangeId, modificationChangeId, reviseChange: true);
        ...
        model.Questions.ShouldContain(q => q.QuestionId == "Q-MAIN");
        model.Questions.ShouldNotContain(q => q.QuestionId == "Q-ORG");

        Sut.TempData[TempDataKeys.ProjectModification.SpecificAreaOfChangeText].ShouldBe("Add new sites");
        Sut.TempData[TempDataKeys.ProjectModification.SpecificAreaOfChangeId].ShouldBe(specificAreaOfChangeId);
        Sut.TempData[TempDataKeys.ProjectModification.ProjectModificationChangeId].ShouldBe(modificationChangeId);

        _respondentService.Verify(s => s.GetModificationParticipatingOrganisationAnswers(participatingOrganisationId), Times.Once);
        _validator.Verify(v => v.ValidateAsync(It.IsAny<ValidationContext<QuestionnaireViewModel>>(), It.IsAny<CancellationToken>()), Times.Exactly(2));
    }
