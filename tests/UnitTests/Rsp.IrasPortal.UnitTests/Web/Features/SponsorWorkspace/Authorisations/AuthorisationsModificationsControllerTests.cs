...
public class AuthorisationsModificationsControllerTests : TestServiceBase<TestAuthorisationsModificationsController>
{
    ...
    [Fact]
    public async Task CheckAndAuthorise_Returns_FailureResult_When_Authorisation_Fails()
    {
        SetupAuthoriseOutcomeViewModel();
    ...
    [Fact]
    public async Task CheckAndAuthorise_ValidModelState_ReviseAndAuthorise_Should_Redirect_To_ModificationDetails()
    {
        // Arrange
        var viewModel = SetupAuthoriseOutcomeViewModel();
        viewModel.Outcome = "ReviseAndAuthorise";

        var modificationId = Guid.Parse(viewModel.ModificationId);
        var projectRecordId = viewModel.ProjectRecordId;
        var irasId = viewModel.IrasId;
        var shortTitle = viewModel.ShortTitle;
        var sponsorUserId = viewModel.SponsorOrganisationUserId;

        Mocker.GetMock<IProjectModificationsService>()
            .Setup(s => s.UpdateModificationStatus(
                It.Is<UpdateModificationStatusRequest>(r =>
                    r.ProjectRecordId == projectRecordId &&
                    r.ModificationId == modificationId &&
                    r.Status == ModificationStatus.ReviseAndAuthorise &&
                    r.ReasonNotApproved == string.Empty &&
                    r.Response == null &&
                    r.Role == null &&
    ...
    public async Task RequestRevisions_WhenSponsorServiceFails_ReturnsServiceError()
    {
        ...
        Mocker.GetMock<ISponsorOrganisationService>()
            .Setup(s => s.GetSponsorOrganisationUser(Guid.Parse(vm.SponsorOrganisationUserId), vm.RtsId))
            .ReturnsAsync(new ServiceResponse<SponsorOrganisationUserDto>
            {
                StatusCode = HttpStatusCode.BadRequest
            });
    ...
    public async Task SendRequestRevisions_ValidModelState_UpdatesStatus_AndRedirectsToConfirmation()
    {
        ...
        projectModService.Verify(s =>
            s.UpdateModificationStatus(
                It.Is<UpdateModificationStatusRequest>(r =>
                    r.ProjectRecordId == vm.ProjectRecordId &&
                    r.ModificationId == Guid.Parse(vm.ModificationId) &&
    ...
    private AuthoriseModificationsOutcomeViewModel SetupAuthoriseOutcomeViewModel()
    {
        ...
        Sut.TempData[TempDataKeys.ProjectModification.SpecificAreaOfChangeText] = "0c715e3f-22bd-47fb-9a37-710e0248b761";
        ...
        Mocker.GetMock<IProjectModificationsService>()
            ...
            .ReturnsAsync(new ServiceResponse<IEnumerable<ProjectModificationChangeResponse>>
            {
                ...
                [
                    ...
                    {
                        Id = changeId,
                        SpecificAreaOfChange = "192152cb-db19-447c-b410-aa394f1de54c",
                        AreaOfChange = "A1",
                        Status = ModificationStatus.InDraft,
                    }
                ]
                    ...
                    [
                        new AreaOfChangeDto
                        {
                            ...
                            SpecificAreasOfChange =
                                [new AnswerModel { AutoGeneratedId = "192152cb-db19-447c-b410-aa394f1de54c", OptionName = "Temporary halt to a project" }]
                        },
                        new AreaOfChangeDto
                        {
                            AutoGeneratedId = "A1",
                            OptionName = "Area Name",
                            SpecificAreasOfChange =
                                [new AnswerModel { AutoGeneratedId = "0c715e3f-22bd-47fb-9a37-710e0248b761", OptionName = "Specific Name" }]
                        }
                    ]
                }
            });
        ...
        projectModService.Verify(s =>
             s.UpdateModificationStatus(
                 It.Is<UpdateModificationStatusRequest>(r =>
                     r.ProjectRecordId == vm.ProjectRecordId &&
                     r.ModificationId == Guid.Parse(vm.ModificationId) &&
                     r.Status == ModificationStatus.NotAuthorised
                 )),
             Times.Once);
    ...
    [Fact]
    public async Task AuthoriseRevision_InvalidModel_Redirects_To_ModificationDetails()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        model.Outcome = "ReviseAndAuthorise"; // overridden anyway

        Mocker.GetMock<IValidator<AuthoriseModificationsOutcomeViewModel>>()
            .Setup(v => v.ValidateAsync(It.IsAny<ValidationContext<AuthoriseModificationsOutcomeViewModel>>(), default))
            .ReturnsAsync(new ValidationResult
            {
                Errors = { new ValidationFailure("RevisionDescription", "required") }
            });
        ...
        redirect.RouteValues!["irasId"].ShouldBe(model.IrasId);
        redirect.RouteValues!["shortTitle"].ShouldBe(model.ShortTitle);
        redirect.RouteValues!["projectModificationId"].ShouldBe(Guid.Parse(model.ModificationId));
        redirect.RouteValues!["sponsorOrganisationUserId"].ShouldBe(model.SponsorOrganisationUserId);
        redirect.RouteValues!["rtsId"].ShouldBe(model.RtsId);

        Sut.TempData.ContainsKey(TempDataKeys.ModelState).ShouldBeTrue();
    }

    [Fact]
    public async Task AuthoriseRevision_SaveForLater_SkipsValidation_And_Redirects_To_Modifications()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        model.RevisionDescription = ""; // ensures skipValidation = true
        ...
        Mocker.GetMock<IProjectModificationsService>()
            .Setup(s => s.UpdateModificationStatus(
                It.Is<UpdateModificationStatusRequest>(r =>
                    ...
                    r.Status == ModificationStatus.ReviseAndAuthorise &&
                    r.ReasonNotApproved == model.RevisionDescription &&
                    r.Response == null &&
                    r.Role == null
                )))
            .ReturnsAsync(new ServiceResponse
        ...
        var redirect = result.ShouldBeOfType<RedirectToActionResult>();
        redirect.ActionName.ShouldBe(nameof(AuthorisationsModificationsController.Modifications));
    ...
    [Fact]
    public async Task AuthoriseRevision_WithExistingReviewBody_Redirects_To_CanSubmitToReviewBody()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        ...
        Mocker.GetMock<IProjectModificationsService>()
            ...
            .ReturnsAsync(new ServiceResponse<GetModificationsResponse>
            {
                ...
                Content = new GetModificationsResponse
                {
                    Modifications =
                    [
                        new ModificationsDto { Status = ModificationStatus.WithReviewBody }
                    ]
                }
            });
        ...
        result.ShouldBeOfType<RedirectToActionResult>()
              .ActionName.ShouldBe(nameof(AuthorisationsModificationsController.CanSubmitToReviewBody));
    }

    [Fact]
    public async Task AuthoriseRevision_ReviewRequired_SetsStatus_WithReviewBody_And_Redirects_To_Confirmation()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        model.ReviewType = "Review required";
        model.ModificationChanges = [
        ...
        Mocker.GetMock<IProjectModificationsService>()
            .Setup(s => s.UpdateModificationStatus(
                It.Is<UpdateModificationStatusRequest>(r =>
                    r.ProjectRecordId == model.ProjectRecordId &&
                    r.ModificationId == Guid.Parse(model.ModificationId) &&
                    r.Status == ModificationStatus.WithReviewBody &&
                    r.ReasonNotApproved == model.RevisionDescription
                )))
            .ReturnsAsync(new ServiceResponse
            {
    ...
    [Fact]
    public async Task AuthoriseRevision_NoReviewRequired_SetsStatus_Approved_And_Redirects_To_Confirmation()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        ...
        Mocker.GetMock<IProjectModificationsService>()
            .Setup(s => s.UpdateModificationStatus(
                It.Is<UpdateModificationStatusRequest>(r =>
                    r.ProjectRecordId == model.ProjectRecordId &&
                    r.ModificationId == Guid.Parse(model.ModificationId) &&
                    r.Status == ModificationStatus.Approved &&
                    r.ReasonNotApproved == model.RevisionDescription
                )))
            .ReturnsAsync(new ServiceResponse
            {
        ...
        Mocker.GetMock<IProjectModificationsService>()
            .Setup(s => s.UpdateModificationStatus(
                It.Is<UpdateModificationStatusRequest>(r =>
                    ...
                    r.Response == null &&
                    r.Role == null
                // ResponseOrigin intentionally not constrained (was It.IsAny<string?>())
                )))
            .ReturnsAsync(new ServiceResponse
            {
                StatusCode = HttpStatusCode.OK
    ...
    [Fact]
    public async Task AuthoriseRevision_ProjectHalt_UpdateStatus_Fail()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        model.ModificationChanges = [
                        new ModificationChangeModel
    ...
    [Fact]
    public async Task AuthoriseRevision_ProjectRestart_UpdateStatus_And_Redirects_To_Confirmation()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        model.ModificationChanges = [
                 new ModificationChangeModel
                 {
                     SpecificAreaOfChangeId ="0c715e3f-22bd-47fb-9a37-710e0248b761"
                 },
        ];
        model.ReviewType = "Review required";
    ...
    [Fact]
    public async Task AuthoriseRevision_ProjectRestart_UpdateStatus_Update_Fail()
    {
        var model = SetupAuthoriseOutcomeViewModel();
        model.ModificationChanges = [
                 new ModificationChangeModel
    ...
    public async Task AuthoriseRevision_ProjectHalt_UpdateStatus_Fails()
    {
        ...
        var tempData = new TempDataDictionary(new DefaultHttpContext(), Mock.Of<ITempDataProvider>())
        {
            [TempDataKeys.ProjectModification.SpecificAreaOfChangeText] = "192152cb-db19-447c-b410-aa394f1de54c"
        };
        Sut.TempData = tempData;
        Mocker.GetMock<IValidator<AuthoriseModificationsOutcomeViewModel>>()
            .Setup(v => v.ValidateAsync(It.IsAny<ValidationContext<AuthoriseModificationsOutcomeViewModel>>(), default))
    ...
    [Fact]
    public async Task CheckAndAuthorise_ValidModelState_Update_ProjectHalt_Status()
    {
        // Arrange
        var authoriseOutcomeViewModel = SetupAuthoriseOutcomeViewModel();
        var tempData = new TempDataDictionary(new DefaultHttpContext(), Mock.Of<ITempDataProvider>())
    ...
    [Fact]
    public async Task CheckAndAuthorise_ValidModelState_Update_ProjectHalt_Status_Fail()
    {
        // Arrange
        var authoriseOutcomeViewModel = SetupAuthoriseOutcomeViewModel();
        authoriseOutcomeViewModel.ModificationChanges = [
                                 new ModificationChangeModel
                                 {
                                     SpecificAreaOfChangeId ="192152cb-db19-447c-b410-aa394f1de54c"
                                 },
