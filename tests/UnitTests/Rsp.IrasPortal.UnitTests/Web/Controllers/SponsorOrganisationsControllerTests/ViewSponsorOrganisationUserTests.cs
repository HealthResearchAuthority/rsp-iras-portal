...
public class ViewSponsorOrganisationUserTests : TestServiceBase<SponsorOrganisationsController>
{
    private readonly DefaultHttpContext _http;

    public ViewSponsorOrganisationUserTests()
    {
    ...
    [Theory]
    [AutoData]
    public async Task ViewSponsorOrganisationUser_AddUser_ReturnsToView(
        SponsorOrganisationUserDto sponsorOrganisationUserDto)
    {
        ...
        // Act
        var result = await Sut.ViewSponsorOrganisationUser(rtsId, userGuid, true);

        // Assert
        var view = result.ShouldBeOfType<ViewResult>();
        view.ViewData["type"].ShouldBe("add");
    }

    [Theory]
    [AutoData]
    public async Task ViewSponsorOrganisationUser_EditUser_ReturnsToView(
        SponsorOrganisationUserDto sponsorOrganisationUserDto)
    {
        // Arrange
        const string rtsId = "87765";
        ...
        // Act
        var result = await Sut.ViewSponsorOrganisationUser(rtsId, userGuid);

        // Assert
        var view = result.ShouldBeOfType<ViewResult>();
        view.ViewData["type"].ShouldBe("edit");
    }
}