...
public class ModificationDetailsPageTests : TestServiceBase<ProjectModificationController>
{
    ...
    public void ModificationDetailsPage_ReturnsViewWithPopulatedModel()
    {
        ...
        var model = viewResult.Model.ShouldBeOfType<ModificationDetailsPageViewModel>();
        model.ModificationChanges.ShouldNotBeNull();
        model.ModificationChanges.ToList().Count.ShouldBe(2);
        model.Status.ShouldBe("Draft");
        model.ModificationType.ShouldBe("Minor modification");
    }
}