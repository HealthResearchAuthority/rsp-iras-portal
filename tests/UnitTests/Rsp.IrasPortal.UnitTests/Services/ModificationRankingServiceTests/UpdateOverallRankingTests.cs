...
public class UpdateOverallRankingTests : TestServiceBase<ModificationRankingService>
{
    ...
    public async Task Does_Nothing_When_Modification_Not_Found()
    {
        ...
        Mocker
            ...
            .ReturnsAsync(new ServiceResponse<ProjectModificationResponse> { StatusCode = System.Net.HttpStatusCode.NotFound });
    ...
    public async Task Aggregates_Change_Rankings_To_Overall_Modification()
    {
        ...
        Mocker
            .GetMock<IProjectModificationsService>()
            .Setup(s => s.GetModification(modId))
            .ReturnsAsync(new ServiceResponse<ProjectModificationResponse> { StatusCode = System.Net.HttpStatusCode.OK, Content = new ProjectModificationResponse { Id = modId } });

        Mocker
            .GetMock<IProjectModificationsService>()
            .Setup(s => s.GetModificationChanges(modId))
            .ReturnsAsync(new ServiceResponse<IEnumerable<ProjectModificationChangeResponse>> { StatusCode = System.Net.HttpStatusCode.OK, Content = new[] { change1, change2 } });

        Mocker
            .GetMock<ICmsQuestionsetService>()
            .Setup(s => s.GetInitialModificationQuestions())
            .ReturnsAsync(new ServiceResponse<StartingQuestionsDto> { StatusCode = System.Net.HttpStatusCode.OK, Content = new StartingQuestionsDto { AreasOfChange = [new AreaOfChangeDto { AutoGeneratedId = "A1", SpecificAreasOfChange = [new AnswerModel { AutoGeneratedId = "SA1", ShowApplicabilityQuestions = true }, new AnswerModel { AutoGeneratedId = "SA2", ShowApplicabilityQuestions = true }] }] } });

        Mocker
            .GetMock<ICmsQuestionsetService>()
            .Setup(s => s.GetModificationsJourney(It.IsAny<string>()))
            .ReturnsAsync(new ServiceResponse<CmsQuestionSetResponse> { StatusCode = System.Net.HttpStatusCode.OK, Content = new CmsQuestionSetResponse { Sections = [new SectionModel { Questions = [] }] } });
